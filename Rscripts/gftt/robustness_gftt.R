...
gftt <- gftt %>% filter(input_id == test_id)
...
p <- ggplot(data=results, aes(x=input_id,y=robustness, group=id,color=id, fill=id), size=1.2) + 
  ...
  labs(x="image ID", y=expression("Robustness R"[x]), color="GFTT\nImage degradation", fill="GFTT\nImage degradation") + 
  theme_bw() + 
...
p
ggsave("./robustness_gftt.pdf", plot = p,width = 5.79, height = 4.65, dpi=300, device=cairo_pdf)
