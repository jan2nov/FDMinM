...
patient2 = list.files(path = "../data/methods_testing/orb/orig/6-6/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient2))
patient3 = list.files(path = "../data/methods_testing/orb/orig/5-5/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
...
patient5 = list.files(path = "../data/methods_testing/orb/orig/3-3/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient5))
patient6 = list.files(path = "../data/methods_testing/orb/orig/2-2/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient6))
patient7 = list.files(path = "../data/methods_testing/orb/orig/1-1/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
...
write.table(n_keypoints, "orig-orb_n_keypoints.dat", append = FALSE, sep = " ",dec = ".",
...
for(i in 1:length(patient2)){
  ...
  for(j in 1:length(matches)){
    add_line <- c(i,j,matches[j], percentage[j])
    results3 <- rbind(results3,add_line)
  }
}
...
#                  color=as.factor(x), 
  ...
  labs(colour = "Image #", x = "Image ID = position [1mm]", y="Number of matches [%]", title = "Method: ORB, comparing images -- same patient")
