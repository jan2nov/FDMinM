...
lengths <- NULL
patient = list.files(path = "../data/methods_testing/akaze/noise/gauss/10/7-7/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient))
patient2 = list.files(path = "../data/methods_testing/akaze/noise/gauss/10/6-6/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient2))
patient3 = list.files(path = "../data/methods_testing/akaze/noise/gauss/10/5-5/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient3))
patient4 = list.files(path = "../data/methods_testing/akaze/noise/gauss/10/4-4/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient4))
patient5 = list.files(path = "../data/methods_testing/akaze/noise/gauss/10/3-3/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient5))
patient6 = list.files(path = "../data/methods_testing/akaze/noise/gauss/10/2-2/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient6))
patient7 = list.files(path = "../data/methods_testing/akaze/noise/gauss/10/1-1/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
...
write.table(n_keypoints, "gauss-10-akaze_n_keypoints.dat", append = FALSE, sep = " ",dec = ".",
...
write.table(test, "gauss-10-akaze_results.dat", append = FALSE, sep = " ",dec = ".",
          row.names = FALSE, col.names = TRUE, quote = FALSE)
...
ggplot(temp3, aes(x=y, 
  ...
  labs(colour = "Image #", fill= "Image #", x = "Image ID = position [1mm]", y="Number of matches [%]", title = "Method: AKAZE, comparing images -- same patient; Gauss -- 10 STD")
