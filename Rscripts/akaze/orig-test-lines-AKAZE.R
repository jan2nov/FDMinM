...
patient2 = list.files(path = "../data/methods_testing/akaze/orig/6-6/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient2))
patient3 = list.files(path = "../data/methods_testing/akaze/orig/5-5/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
...
patient5 = list.files(path = "../data/methods_testing/akaze/orig/3-3/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient5))
patient6 = list.files(path = "../data/methods_testing/akaze/orig/2-2/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient6))
patient7 = list.files(path = "../data/methods_testing/akaze/orig/1-1/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
...
write.table(n_keypoints, "orig-akaze_n_keypoints.dat", append = FALSE, sep = " ",dec = ".",
            row.names = FALSE, col.names = TRUE, quote = FALSE)


number <- length(patient)%/%170
results2 <- data.frame()
for(j in 1:number){
  print(j)
...
#                  color=as.factor(x), 
  ...
  labs(colour = "Image #", fill= "Image #", x = "Image ID = position [1mm]", y="Number of matches [%]", title = "Method: AKAZE, comparing images -- same patient")
