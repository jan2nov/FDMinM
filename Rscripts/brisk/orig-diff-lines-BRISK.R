...
patient <- c(patient, patient2, patient3, patient4, patient5, patient6)
...
write.table(n_keypoints, "orig-diff-brisk_n_keypoints.dat", append = FALSE, sep = " ",dec = ".",
...
write.table(test, "orig-diff-brisk_results.dat", append = FALSE, sep = " ",dec = ".",
            row.names = FALSE, col.names = TRUE, quote = FALSE)
...
ggplot(temp, aes(x=y, y=percentage,color=as.factor(x))) +
  ...
  labs(colour = "Image #", x = "Image ID = position [1mm]", y="Number of matches [%]", title = "Comparing images with etalon")
...
ggplot(temp3, aes(x=y, 
  ...
  labs(colour = "Image #", fill= "Image #", x = "Image ID = position [1mm]", y="Number of matches [%]", title = "Method: BRISK, comparing images with etalon")
