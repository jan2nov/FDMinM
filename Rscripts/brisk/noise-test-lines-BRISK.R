...
lengths <- NULL
patient = list.files(path = "../data/methods_testing/brisk/noise/gauss/10/7-7/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient))
patient2 = list.files(path = "../data/methods_testing/brisk/noise/gauss/10/6-6/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient2))
patient3 = list.files(path = "../data/methods_testing/brisk/noise/gauss/10/5-5/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient3))
patient4 = list.files(path = "../data/methods_testing/brisk/noise/gauss/10/4-4/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient4))
patient5 = list.files(path = "../data/methods_testing/brisk/noise/gauss/10/3-3/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient5))
patient6 = list.files(path = "../data/methods_testing/brisk/noise/gauss/10/2-2/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient6))
patient7 = list.files(path = "../data/methods_testing/brisk/noise/gauss/10/1-1/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
...
for (i in 1:length(lengths)){
  filename = patient[(i-1)*170 + 1]
  data = read.table(filename,header = FALSE, fill = TRUE, sep = " ")
  colnames(data) <- c("test_z", "ref_z", "keypoints_refImage", "keypoints_testedImage", "n_matches")
  add_line <- data.frame(id=data$ref_z,x=data$keypoints_testedImage)  
  mean_number_keypoints <- rbind(mean_number_keypoints,add_line)
}
n_keypoints <- do.call("data.frame", aggregate(mean_number_keypoints$x,list(y=mean_number_keypoints$id),function(x) c(mean=round(mean(x)),sd=round(sd(x)))))
...
write.table(n_keypoints, "noise-brisk_n_keypoints.dat", append = FALSE, sep = " ",dec = ".",
...
write.table(test, "noise-10-brisk_results.dat", append = FALSE, sep = " ",dec = ".",
          row.names = FALSE, col.names = TRUE, quote = FALSE)
...
ggplot(temp3, aes(x=y, 
  ...
  labs(colour = "Image #", fill= "Image #", x = "Image ID = position [1mm]", y="Number of matches [%]", title = "Method: BRISK, comparing images -- same patient; Gauss -- 10 STD")
