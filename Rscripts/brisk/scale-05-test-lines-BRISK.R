...
patient2 = list.files(path = "../data/methods_testing/brisk/scale/6-6/05/larger/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient2))
patient3 = list.files(path = "../data/methods_testing/brisk/scale/5-5/05/larger/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient3))
patient4 = list.files(path = "../data/methods_testing/brisk/scale/4-4/05/larger/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient4))
patient5 = list.files(path = "../data/methods_testing/brisk/scale/3-3/05/larger/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient5))
patient6 = list.files(path = "../data/methods_testing/brisk/scale/2-2/05/larger/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient6))
patient7 = list.files(path = "../data/methods_testing/brisk/scale/1-1/05/larger/", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
...
write.table(n_keypoints, "scale-05-brisk_n_keypoints.dat", append = FALSE, sep = " ",dec = ".",
...
write.table(test, "scale-05-brisk_results.dat", append = FALSE, sep = " ",dec = ".",
          row.names = FALSE, col.names = TRUE, quote = FALSE)
...
ggplot(temp3, aes(x=y, 
  ...
  labs(colour = "Image #", fill= "Image #", x = "Image ID = position [1mm]", y="Number of matches [%]", title = "Method: BRISK, comparing images -- same patient; Scale -- 5 %")
