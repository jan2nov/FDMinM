...
patient2 = list.files(path = "../data/methods_testing/brisk/rotation/05/6-6", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient2))
patient3 = list.files(path = "../data/methods_testing/brisk/rotation/05/5-5", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient3))
patient4 = list.files(path = "../data/methods_testing/brisk/rotation/05/4-4", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient4))
patient5 = list.files(path = "../data/methods_testing/brisk/rotation/05/3-3", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient5))
patient6 = list.files(path = "../data/methods_testing/brisk/rotation/05/2-2", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
lengths <- c(lengths,length(patient6))
patient7 = list.files(path = "../data/methods_testing/brisk/rotation/05/1-1", pattern = ".txt",include.dirs = TRUE,full.names = TRUE)
...
write.table(n_keypoints, "rotation-05-brisk_n_keypoints.dat", append = FALSE, sep = " ",dec = ".",
...
write.table(test, "rotation-05-brisk_results.dat", append = FALSE, sep = " ",dec = ".",
          row.names = FALSE, col.names = TRUE, quote = FALSE)
...
ggplot(temp3, aes(x=y, 
  ...
  labs(colour = "Image #", fill= "Image #", x = "Image ID = position [1mm]", y="Number of matches [%]", title = "Method: BRISK, comparing images -- same patient; rotation -- 5°")
